...
namespace CEF.Common.Extentions
{
    public static partial class Extention
    {
        ...
        /// <param name="placeholderLen">占位符</param>
        /// <returns></returns>
        public static List<string> SmsContentSplit(this string value, int length, int placeholderLen)
        {
            ...
            {
                list.Add(value);
                for (int i = 0; i < placeholderLen - 1; i++)
                {
                    list.Add("");
                }
            }
            else
            {
                ...
                int arraySize = (totalLength / length) + 1;

                if (totalLength % placeholderLen == 0)
                {
                    arraySize = (totalLength / length);
                    for (int i = 0; i < arraySize; i++)
                    {
                        list.Add(value.Substring(i * length, length));
                    }

                    var listLen = list.Select(p => p.Length).Sum();
                    if (totalLength > listLen)
                    {
                        list.Add(value.Substring(listLen, totalLength - listLen));
                    }
                }
                else
                {
                    arraySize = (totalLength / length) + 1;
                    for (int i = 0; i < arraySize; i++)
                    {
                        if (i != arraySize - 1)
                        {
                        ...
                        {
                            list.Add(value.Substring(i * length, totalLength % length));
                        }
                    }
                }
                var remainCnt = placeholderLen - list.Count();
                if (remainCnt > 0)
                {
