...
using CEF.Common.AOP.Abstraction;
...
namespace CEF.Common.Extentions
{
    ...
    public static partial class Extention
    {
        ...
        public static IServiceCollection AddAutoMapper(this IServiceCollection services, Action<IMapperConfigurationExpression> configure = null)
        {
            ...
            maps.AddRange(GlobalConfigure.AllTypes.Where(x => x.GetCustomAttribute<MapAttribute>() != null)
            ...
            var configuration = new MapperConfiguration(cfg =>
            {
                ...
                cfg.AddMaps(GlobalConfigure.AllAssemblies);
        ...
        public static IServiceCollection AddFxServices(this IServiceCollection services)
        {
            ...
            GlobalConfigure.AllTypes.ForEach(aType =>
            {
                lifeTimeMap.ToList().ForEach(aMap =>
                {
                    ...
                    if (theDependency.IsAssignableFrom(aType) && theDependency != aType && !aType.IsAbstract && aType.IsClass)
                    {
                        ...
                        var interfaces = GlobalConfigure.AllTypes.Where(x => x.IsAssignableFrom(aType) && x.IsInterface && x != theDependency).ToList();
                        //有接口则注入接口
                        if (interfaces.Count > 0)
                        {
                            interfaces.ForEach(aInterface =>
                            {
                                //services.Add(new ServiceDescriptor(aInterface, aType, aMap.Value));
                                //注入AOP
